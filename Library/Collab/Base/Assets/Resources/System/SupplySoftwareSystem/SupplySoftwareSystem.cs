    ...
    */
    ...
    public static void BaseSoft(string software)//몇레벨 장비 뽑을건지
    {
        ...
        if (isSatellite)
        {
            ...
            Instantiate(baseSoft, satellites.transform);
            satellites.transform.Find(baseSoft.name + "(Clone)").gameObject.GetComponent<Software>().level++;
        }
        else
        {
            baseSoft = Resources.Load("System/Software/" + software + "/" + software) as GameObject;
            Instantiate(baseSoft, softwares.transform);
            softwares.transform.Find(baseSoft.name + "(Clone)").gameObject.GetComponent<Software>().level++;
        }
    }
